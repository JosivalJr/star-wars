...
export default function StarshipCard({ cardInfos, toggleModal }: Props) {
  const backgroundFileName = sanitizeImageName(cardInfos.name);
  const initialBackgroundImage = `${
    import.meta.env.VITE_BASE_URL
  }assets/images/starships/${backgroundFileName}.jpg`;
  ...
  const handleImageError = () => {
    setBackgroundImage(
      `${import.meta.env.VITE_BASE_URL}assets/images/starships/default.png`
    );
  };
      ...
      >
        ...
        <div className="relative z-10">
          ...
          <p className="pt-10 text-2xl capitalize leading-10">
            ...
            {cardInfos.passengers == "n/a" || cardInfos.passengers == "unknown"
              ? "Desconhecido"
              : numberDotMask(Number(cardInfos.passengers))}
            <br />
            <span className="text-blue-400">Max Speed: </span>
            {cardInfos.max_atmosphering_speed == "n/a" ||
            cardInfos.max_atmosphering_speed == "unknown"
              ? "Desconhecido"
              : cardInfos.max_atmosphering_speed}
            <br />
            <br />
          </p>
        </div>
